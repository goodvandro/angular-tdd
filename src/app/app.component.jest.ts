...
const server = setupServer(
  ...
  rest.get('/api/1.0/users/:id', (req, res, ctx) => {
    ...
    return res(
      ctx.status(200),
      ctx.json({ id, username: `user${id}`, email: `user${id}@mail.com` })
    );
  }),
...
describe('Routing', () => {
  ...
  it.each`
    ...
    ${'/signup'} | ${'Sign Up'}
    ${'/login'}  | ${'Login'}
  `('has link title $title to $path', async ({ path, title }) => {
    await setup(path);
  ...
  it('navigate to user page when clicking the username on user list', async () => {
    await setup('/');
...
describe('Login', () => {
  ...
  it('clears storage after user logout', async () => {
    await setupForm();
    await userEvent.click(button);
    const logoutLink = await screen.findByText('Logout');
    await userEvent.click(logoutLink);
